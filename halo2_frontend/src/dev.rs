...
impl<F: FromUniformBytes<64> + Ord> MockProver<F> {
    ...
    pub fn verify_at_rows<I: Clone + Iterator<Item = usize>>(
       ...
       // let mut cached_cq_table = Vec::new();
        //let mut cached_cq_table_identifier = Vec::new();
        // Check that all lookups exist in their respective tables.
        let cq_lookup_errors =
            self.cs
                .cq_lookups
                .iter()
                .enumerate()
                .flat_map(|(lookup_index, lookup)|  {
                    assert!(lookup.table_ids.len() == lookup.input_expressions.len());
                    assert!(self.usable_rows.end > 0);


                    let temp_table = &lookup.vec_columns.iter().map(|vec|
						{vec.into_iter().map(|x| Value::Real(*x))
							.collect::<Vec<Value<_>>>()
						}).collect::<Vec<Vec<Value<_>>>>();
					let n = temp_table[0].len();
					let m = temp_table.len();
					let mut table = vec![];
					for j in 0..m{
						assert!(temp_table[j].len()==temp_table[0].len(), "cq_table lookup columns len() not the same!");
					}
					for i in 0..n{
						let mut tuple = vec![];
						for j in 0..m{ tuple.push(temp_table[j][i]); }
						table.push(tuple);
					}
