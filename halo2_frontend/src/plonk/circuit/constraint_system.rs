...
use crate::plonk::{
    lookup, cq_lookup, permutation, shuffle, Advice, AdviceQuery, Challenge, Column, Expression, FirstPhase,
    Fixed, FixedQuery, Instance, InstanceQuery, Phase, Selector, TableColumn, CqTableColumn
};
use core::cmp::max;
    ...
    fn from(cs: ConstraintSystem<F>) -> Self {
        ConstraintSystemMid {
            ...
            cq_lookups: cs
                .cq_lookups
                .into_iter()
                .map(|l| halo2_middleware::cq_lookup::ArgumentMid {
                    name: l.name,
                    input_expressions: l.input_expressions.into_iter().map(|e| e.into()).collect(),
                    table_ids: l.table_ids,
					vec_columns: l.vec_columns,
					hs_idx: l.hs_idx,
                })
                .collect(),
            shuffles: cs
...
pub struct ConstraintSystem<F: Field> {
    ...
    // Vector of cq_lookup arguments, where each corresponds to a sequence of
    // input expressions and a sequence of cq tables involved in the lookup.
	// we perform a folded (vector) lookup if there are multiple tuples.
    pub(crate) cq_lookups: Vec<cq_lookup::Argument<F>>,
...
impl<F: Field> ConstraintSystem<F> {
    ...
    /// Add a cq_lookup argument for some input expressions and table columns.
    ///
    /// `table_map` returns a map between input expressions and the table 
	/// columns they need to match.
    pub fn cq_lookup<S: AsRef<str>>(
        &mut self,
        name: S,
        table_map: impl FnOnce(&mut VirtualCells<'_, F>) -> Vec<(Expression<F>, CqTableColumn)>,
    ) -> usize {
        let mut cells = VirtualCells::new(self);
        let table_map = table_map(&mut cells)
            .into_iter()
            .map(|(mut input, table)| {
                ...
                input.query_cells(&mut cells);
                (input, table.cache_id)
            })
            .collect();
        let index = self.cq_lookups.len();

        self.cq_lookups
            .push(cq_lookup::Argument::new(name.as_ref(), table_map));
    ...
    fn replace_selectors_with_fixed(&mut self, selector_replacements: &[Expression<F>]) {
        ...
        for expr in self.cq_lookups.iter_mut().flat_map(|lookup| {
            lookup
                .input_expressions
                .iter_mut()
        }) {
    ...
    /// Allocates a new CqTableColumn. Note that each CqTableColumn
	/// has a UNIQUE id in the cache/ folder. The constructor needs that UNIQUE id.
    pub fn cq_lookup_table_column(&mut self, cache_id: usize) -> CqTableColumn {
        CqTableColumn {cache_id: cache_id}
    }
    ...
    pub fn blinding_factors(&self) -> usize {
        ...
        // off-by-one errors.
		// Another +1 for the blinding factor for cq_lookup
        factors + 1 + 1 
    }
