...
/// The fixed column for Cq lookup. Holds very large table that are
/// not decomposable like those in Lasso. CqTableColumn does not
/// involve in any constraint computation, but only used for cq lookup.
/// The commitment is computed outside halo2 prover, and the proof is done
/// outside of halo2 prover.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash, Ord, PartialOrd)]
pub struct CqTableColumn{
	/// the id to load cached contents from cache/ directory
	pub cache_id: usize, 
}

impl CqTableColumn{
	/// return the cache_id to load the contents from cache/ directory
    pub fn cache_id(&self) -> usize{self.cache_id}
}
