...
> {
	compile_circuit_cq
		::<F, ConcreteCircuit>(k, circuit, compress_selectors, None)
}

/// random_selectors_for_fixed used to blind selector columns
/// this is because compile_circuit might be called multiple times, must ensure
/// the same blinders are used in the selector (fixed) columns.
#[allow(clippy::type_complexity)]
pub fn compile_circuit_cq<F: Field, ConcreteCircuit: Circuit<F>>(
    k: u32,
    circuit: &ConcreteCircuit,
    compress_selectors: bool,
	selector_blinders: Option<&Vec<F>>,
) -> Result<
    (
        CompiledCircuit<F>,
        ConcreteCircuit::Config,
...
> {
	...
	// PATCH: it looks like in complex_selector mode the
	// fixed columns have already been added for complex selectors
	// but later fixed is extended with selector_polys which creates
	// additional column. In this case adjust the intial columns
	// created
	let num_fix_col = if compress_selectors {cs.num_fixed_columns - cs.selector_map.len()} else {cs.num_fixed_columns};
    let mut assembly = plonk::keygen::Assembly {
        k,
        fixed: vec![vec![F::ZERO.into(); n]; num_fix_col],
    ...
    let mut selector_polys = selectors_to_fixed.convert(assembly.selectors);
	if selector_polys.len()>0 && !selector_blinders.is_none(){
		let num_blinders = cs.blinding_factors();
		let blinders = selector_blinders.unwrap();
		assert!(blinders.len()>=selector_polys.len() * num_blinders,
			"selector_blinders.len(): {} needs >= num_selectors: {} * num_blinders: {}", blinders.len(), selector_polys.len(), num_blinders);
		let n_poly = selector_polys[0].len();	
		let mut idx = 0;
		for i in 1..selector_polys.len(){
			for j in 0..num_blinders{
				selector_polys[i][n_poly-j-1] = blinders[idx+1].clone();
				idx += 1;
			}
		}
	}
