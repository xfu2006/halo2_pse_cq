...
use crate::plonk::{
	...
	cq_lookup::verifier::{cq_lookup_read_compressed_commitments,CqVerifierScheme},
...
pub fn verify_proof_single<
	...
	CV: CqVerifierScheme<F=Scheme::Scalar,Curve=Scheme::Curve,EC=E, TR=T>  
>(
...
{
    verify_proof::<Scheme,V,E,T,Strategy,CV>(params, vk, strategy, &[instance], transcript)
}
...
where
    Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
{
	verify_proof_cq::<Scheme,V,E,T,Strategy,CV> (params, vk, strategy, instances, transcript, None)
}


/// Returns a boolean indicating whether or not the proof is valid
pub fn verify_proof_cq<
    'params,
    Scheme: CommitmentScheme,
    V: Verifier<'params, Scheme>,
    E: EncodedChallenge<Scheme::Curve>,
    ...
    Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
{
    ...
    let cq_lookups_committed= (0..num_proofs)
        .map(|_| -> Result<Vec<_>, _> {
            // Hash each lookup permuted commitment
            vk.cs
                .cq_lookups
                .iter()
                .map(|argument| 
					cq_lookup_read_compressed_commitments(transcript, &cq_ver, &argument.table_ids, *theta))
                	.collect::<Result<Vec<_>, _>>()
        })
        .collect::<Result<Vec<_>, _>>()?;
	for vec_cq_com in &cq_lookups_committed{
		for cq_com in vec_cq_com{
			if !cq_com.cq_ver_res {
				return  Err(Error::Other(format!("Fail cq_lookup")));
			}
		}
	}
    ...
    let cq_lookups_evaluated = cq_lookups_committed
        .into_iter()
        .map(|lookups| -> Result<Vec<_>, _> {
            lookups
                .into_iter()
    ...
    let vanishing = {
        ...
        let expressions = advice_evals
            ...
            .flat_map(
                |(((((advice_evals, instance_evals), permutation), lookups), cq_lookups), shuffles)| {
                    ...
                    std::iter::empty()
                        ...
                        .chain(cq_lookups.iter().zip(vk.cs.cq_lookups.iter()).flat_map(
                            move |(p, argument)| {
                                p.expressions(
                                    l_0,
                                    l_last,
    ...
    let queries = instance_commitments
        ...
        .zip(cq_lookups_evaluated.iter())
        .zip(shuffles_evaluated.iter())
        .flat_map(|(((((((instance_commitments, instance_evals), advice_commitments),advice_evals),permutation),lookups),cq_lookups),shuffles)| {
                iter::empty()
                    .chain(
                        V::QUERY_INSTANCE
                    ...
                    .chain(cq_lookups.iter().flat_map(move |p| p.queries(vk, x)))
