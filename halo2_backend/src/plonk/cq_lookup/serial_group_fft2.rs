/**
    ...
    All Rights Reserved.
    Created: 09/02/2023
	The file is ported from the ZeroAsset IZPR Project 
	Port Started: 01/27/2024
	Completed: 01/28/2024

	 *** This code is an ADAPATATION of ark_work EvaluationDomain and FFT code
	 for field elements.  We adapt it to perform FFT on *group* elements.
	 (see ark_works: poly/src/domain/mod.rs and
...
// ---------------------------------------
// region: group version of FFT
// ---------------------------------------

/// Given a vector of group elements:
///	( [a_1]_1, ..., [a_n]_1 ),
///	the function returns an array
///	( [b_1]_1, ..., [b_n]_1 )
///	vector b is the result of running standard FFT over vector a (field
/// elements)
...
/// Note that this function ACTUALLY runs faster then the group fft
/// because it is essentially field FFT.
///	vec_a = [a_1, ..., a_n]
///	vec_b = standard_fft(vec_a); //say vec_b = [b_1, ..., b_n]
///	return
///	[g^b_1, ..., g^b_n]
///	where g = G::prime_subgroup_generator().
...
/// Similar to group_fft, run inverse IFFT.
pub fn serial_group_ifft<G: AffineCurve>(a: &Vec<G>) -> Vec<G>
where <G as AffineCurve>::Projective: VariableBaseMSM<MSMBase=G, Scalar=<G as AffineCurve>::ScalarField>{
	let n  = a.len();
	assert!(n.is_power_of_two(), "n: {} is not pow of 2", n);
	let mut proj_a: Vec<G::Projective> = a.into_par_iter().map(|x| x.into_projective()). collect::<Vec<G::Projective>>();
	let domain = Radix2GroupEvaluationDomain::<G::Projective>::new(n as u64);
...
// ---------------------------------------
// endregion: group version of FFT
// ---------------------------------------

// ---------------------------------------
// region: assisting data structures adapted from ark_ff
// ---------------------------------------
...
// ---------------------------------------
// endregion: assisting data structures adapted from ark_ff
// ---------------------------------------
...
pub mod tests {
    ...
    use zk_cq::serial_group_fft2::{serial_group_fft, serial_group_ifft, serial_logical_group_fft, serial_logical_group_ifft};
    use zk_cq::ft_poly_ops::*;
