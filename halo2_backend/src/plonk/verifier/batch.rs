...
use crate::{
    ...
    transcript::{Blake2bRead, TranscriptReadBuffer, Challenge255},
};
use crate::plonk::cq_lookup::verifier::{CqVerifierSchemeIPA};
    ...
    pub fn finalize(self, params: &ParamsVerifierIPA<C>, vk: &VerifyingKey<C>) -> bool {
        ...
        let final_msm = self
            ...
            .map(|(i, item)| {
                ...
                let res = verify_proof::<IPACommitmentScheme<C>, VerifierIPA<'_, C>, Challenge255<C>, Blake2bRead<&[u8], C, Challenge255<C>>, BatchStrategy<'_, C>, CqVerifierSchemeIPA<C,Challenge255<C>,Blake2bRead<&[u8],C,Challenge255<C>> > > (params, vk, strategy, &instances, &mut transcript);
