...
use crate::plonk::{
	...
	cq_lookup, cq_lookup::prover::{cq_lookup_commit},
...
use crate::plonk::cq_lookup::{prover::CqProverScheme};
...
pub struct ProverSingle<
	...
	//CP: CqProverScheme<'params, F=Scheme::Scalar,Curve=Scheme::Curve,EC=E, TW=T>
	CP: CqProverScheme
>(Prover<'a, 'params, Scheme, P, E, R, T, M, CP>);
...
impl<
    ...
    > ProverSingle<'a, 'params, Scheme, P, E, R, T, M, CP>
{
    ...
    {
		Self::new_with_engine_cq(engine, params, pk, instance, rng, 
			transcript, &None)
	}

    /// Create a new prover object
    pub fn new_with_engine_cq(
        engine: PlonkEngine<Scheme::Curve, M>,
        params: &'params Scheme::ParamsProver,
        pk: &'a ProvingKey<Scheme::Curve>,
        // TODO: If this was a vector the usage would be simpler
    ...
    where
        Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
    {
        Ok(Self(Prover::new_with_engine_cq(
            engine,
            params,
            pk,
            &[instance],
    ...
    pub fn new(
        ...
        rng: R,
        transcript: &'a mut T,
    ) -> Result<ProverSingle<'a, 'params, Scheme, P, E, R, T, H2cEngine, CP>, Error>
    where
        Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
    {
...
> {
	...
	// Added for supporting Cq LookUp (optional)
	cq_prover: &'params Option<CP>,
}
    ...
    > Prover<'a, 'params, Scheme, P, E, R, T, M, CP>
{
    ...
    where
        Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
    {
		Self::new_with_engine_cq(engine, params, pk, circuits_instances,
			rng, transcript, &None)
	}
        ...
        Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
    {
		...
		if !self.cq_prover.is_none(){//check constraint
 			let cq_blinding = self.cq_prover.as_ref().map(|cp| 
				cp.get_blinding_factors());
			let cs_blinding = pk.vk.cs.blinding_factors();
			assert!(cq_blinding == Some(cs_blinding), "needs to set cq.pkey blinding factor to : {} when calling ParamsKzgCq::setup", cs_blinding);
		}
        let mut cq_lookups_fn =
            |instance: &InstanceSingle<Scheme::Curve>,
             advice: &AdviceSingle<Scheme::Curve, LagrangeCoeff>|
             -> Result<Vec<cq_lookup::prover::CqCommited<Scheme::Curve>>, Error> {
                cs.cq_lookups
                    .iter()
                    .map(|lookup| {
                        cq_lookup_commit(
        ...
        let cq_lookups_committed: Vec<Vec<cq_lookup::prover::CqCommited<Scheme::Curve>>> = instances
            .iter()
            .zip(advices.iter())
            .map(|(instance, advice)| -> Result<Vec<_>, Error> {
                // Construct and commit to permuted values for each lookup
                cq_lookups_fn(instance, advice)
            })
            .collect::<Result<Vec<_>, _>>()?;
        ...
        let cq_lookups_evaluated: Vec<Vec<cq_lookup::prover::CqEvaluated<Scheme::Curve>>> =
           cq_lookups_committed 
                .into_iter()
                .map(|lookups| -> Result<Vec<_>, _> {
                    lookups
        ...
        let queries = instances
            ...
            .zip(cq_lookups_evaluated.iter())
            .zip(shuffles_evaluated.iter())
            .flat_map(|(((((instance, advice), permutation), lookups), cq_lookups), shuffles)| {
                ...
                iter::empty()
                    ...
                    .chain(cq_lookups.iter().flat_map(move |p| p.open(pk, x)))
                    // Shuffles
    ...
    pub fn new(
        ...
        rng: R,
        transcript: &'a mut T,
    ) -> Result<Prover<'a, 'params, Scheme, P, E, R, T, H2cEngine, CP>, Error>
    where
        Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
    {
        let engine = PlonkEngineConfig::build_default();
