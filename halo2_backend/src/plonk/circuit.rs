...
use halo2_middleware::{lookup, permutation::ArgumentMid, shuffle, cq_lookup};
...
pub(crate) type CqLookupArgumentBack<F> = cq_lookup::Argument<F, VarBack>;
...
pub struct ConstraintSystemBack<F: Field> {
	...
	// Vector of CQ lookus
	pub(crate) cq_lookups: Vec<CqLookupArgumentBack<F>>,
...
impl<F: Field> ConstraintSystemBack<F> {
	/// generate the hashmap which from column ID maps to the
	/// CqAux
	pub fn preprocess_cq<E:Engine<Fr=F>>(&self, params: &ParamsKzgCq<E>)
		...
		for cq_arg in &self.cq_lookups{
			for i in 0..cq_arg.table_ids.len(){
				...
				assert!(cap_n==params.pkey.n, "tbl len: {} != params.n: {}",
					cap_n, params.pkey.n);
				let cq_aux = preprocess_cq::<E>(&params.pkey, tbl);
    ...
    pub fn blinding_factors(&self) -> usize {
        ...
        // off-by-one errors. Add one more for cq bliding factor
