...
use crate::{
    ...
    plonk::circuit::{
        ConstraintSystemBack, ExpressionBack, GateBack, LookupArgumentBack, 
		CqLookupArgumentBack,
		QueryBack,
...
use halo2_middleware::{lookup, cq_lookup, poly::Rotation, shuffle};
...
/// Collect queries used in cq_lookups while mapping those lookups to equivalent ones with indexed
/// query references in the expressions.
fn cs_mid_collect_queries_cq_lookups<F: Field>(
    cs_mid: &ConstraintSystemMid<F>,
    queries: &mut QueriesMap,
) -> Vec<CqLookupArgumentBack<F>> {
    cs_mid
        .cq_lookups
        .iter()
        .map(|lookup| cq_lookup::Argument {
            name: lookup.name.clone(),
            input_expressions: lookup
                .input_expressions
                .iter()
                .map(|e| queries.as_expression(e))
                .collect(),
            table_ids: lookup.table_ids.clone(),
			vec_columns: lookup.vec_columns.clone(),
			hs_idx: lookup.hs_idx.clone(),
        })
        .collect()
}
...
fn collect_queries<F: Field>(
    ...
    let cq_lookups = cs_mid_collect_queries_cq_lookups(cs_mid, &mut queries);
    ...
    (queries, gates, lookups, cq_lookups, shuffles)
}

impl<F: Field> From<ConstraintSystemMid<F>> for ConstraintSystemBack<F> {
    fn from(cs_mid: ConstraintSystemMid<F>) -> Self {
        let (queries, gates, lookups, cq_lookups, shuffles) 
			= collect_queries(&cs_mid);
        Self {
...
impl Queries {
    ...
    pub fn blinding_factors(&self) -> usize {
        ...
        // off-by-one errors. 
		//if self.cq_lookups.len()>0 { factors + 1 + 1} else {factors + 1}
		factors + 1
    }
}
